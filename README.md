...
- **Schema 演进**：新增列自动落入目标表；删除列在目标表保留最后一次历史值；INT→BIGINT、VARCHAR 变长等放宽变更静默兼容。类型收窄（BIGINT→INT、VARCHAR 变短导致值溢出）的事件进入死信队列，后续事件继续消费。
- **断点续跑**：位点在目标表写入成功之后提交，写入与提交在主键级临界区内完成，提供 at-least-once 投递；重放时按 version 去重，已落库的行不会重复写入。
- **乱序合并**：同一主键按 version 单调合并，version 低于当前行的事件（含 DELETE）直接丢弃，只推进位点。
- **并发粒度**：每个主键一把锁，不同主键之间不互斥；schema 缓存失效与位点提交在同一临界区内完成，重启后不会用旧列映射解析新事件。
